...
package com.l2g.editor.utils.physics;
...
import com.l2g.editor.utils.math.GeometryUtils;
import com.l2g.editor.utils.math.MathUtils;
import com.l2g.editor.utils.math.ArrayUtils;
import com.l2g.editor.utils.Pair;

import static com.l2g.editor.utils.math.GeometryUtils.filterX;
import static com.l2g.editor.utils.math.GeometryUtils.filterY;
import static com.l2g.editor.utils.math.MathUtils.amplitude2;
import static com.l2g.editor.utils.math.MathUtils.max;
import static com.l2g.editor.utils.math.MathUtils.min;
import static com.l2g.editor.utils.physics.Box2DUtils.Settings.epsilon;
import static com.l2g.editor.utils.physics.Box2DUtils.Settings.linearSlop;
import static com.l2g.editor.utils.physics.Box2DUtils.Settings.maxPolygonVertices;
...
public class Box2DUtils {
	...
	public enum PreconditionCheck {
		...
		public static void checkChainShape(float[] vertices, int offset, int length) {
			...
			for(int i = offset; i + 3 < offset + length; i += 2) {
				...
				if(GeometryUtils.distance2(x1, y1, x2, y2) > Settings.linearSlop * Settings.linearSlop) {
		...
		public static void checkPolygonShape(float[] vertices, int offset, int length) {
			...
			if(length < 6 || length > Settings.maxPolygonVertices * 2)
				throw new InvalidPolygonShapeException("polygon has invalid number of vertices (min: 3, max: Settings.maxPolygonVertices = " + Settings.maxPolygonVertices + "). length: " + length, InvalidPolygonShapeException.Problem.VERTEX_COUNT, vertices, offset, length);
			...
			if(area < Settings.epsilon)
				throw new InvalidPolygonShapeException("polygon area is too small: " + area + " (min is Settings.epsilon: " + Settings.epsilon + ")", InvalidPolygonShapeException.Problem.AREA, vertices, offset, length);
		}
	...
	/** for internal, temporary usage */
	private static final Polygon polygon = new Polygon(new float[Settings.maxPolygonVertices]);
	...
	public static int weld(float[] vertices, int offset, int length) {
		...
		for(int i = offset; i + 1 < offset + length;) {
			...
			if(GeometryUtils.distance2(x1, y1, x2, y2) < Settings.linearSlop / 2) {
